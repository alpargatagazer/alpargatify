...
- `telegram-bot/`: A feature-rich Telegram bot that integrates with Navidrome to deliver scheduled notifications (new albums, anniversaries) and interactive commands (`/search`, `/random`, `/stats`). Includes user authorization, optimized Docker image, and efficient library caching. See `telegram-bot/README.md` for setup and usage.
